...
@triton.jit
def _attn_fwd_inner(acc, l_i, m_i, q, mask,  #
                    desc_k, desc_v,  #
                    offset_y, offset_yk, dtype: tl.constexpr, start_m, qk_scale,  #
                    BLOCK_M: tl.constexpr, HEAD_DIM: tl.constexpr, BLOCK_N: tl.constexpr,  #
                    STAGE: tl.constexpr, offs_m: tl.constexpr, offs_n: tl.constexpr,  #
                    N_CTX: tl.constexpr, N_KV: tl.constexpr, warp_specialize: tl.constexpr, IS_HOPPER: tl.constexpr):

    # range of values handled by this stage
    if STAGE == 1:
        start_mask = tl.cdiv(tl.min(mask).to(tl.int32), BLOCK_M)
    ...
    offsetk_y = offset_yk + lo
    if dtype == tl.float8e5:
        offsetv_y = offset_yk * HEAD_DIM + lo
    else:
        offsetv_y = offset_yk + lo
...
class _attention(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, mask, causal, sm_scale, warp_specialize=False):
        if mask is not None:
            assert causal == True

        if causal == False:
            mask = torch.empty(q.size(0), q.size(2))
